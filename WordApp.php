	...
	function api_to_wordapp( $_post, $post, $context ) {
	 ...
	 function include_wp_rest_api() {
			if ( !in_array( 'json-rest-api/plugin.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
				include_once( trailingslashit( plugin_dir_path( __FILE__ ) ) . 'third/json-rest-api/plugin.php' );
			}
			
				if ( !in_array( 'wp-api-menus/plugin.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
				include_once( trailingslashit( plugin_dir_path( __FILE__ ) ) . 'third/wp-api-menus/plugin.php' );
			}
	}
...
function WordApp_options_enqueue_scripts() {
  //  wp_register_script( 'wptuts-upload', get_template_directory_uri() .'/js/scripts.js', array('jquery') );
